...
/**
 * User: dikmanm
 * Date: 17/10/2015 11:13
 */
@javax.persistence.Entity(name = "Subtitles")
@javax.persistence.Table(name = "subtitles")
...
@NamedQueries({
        ...
        @NamedQuery(name = "Subtitles.getSubtitlesLanguages",
                query = "select distinct u.subtitles from Show as s join s.users as u " +
                        "where u.subtitles is not null and s.id = :showId"),
        @NamedQuery(name = "Subtitles.getSubtitlesLanguagesForTorrent",
                query = "select u.subtitles from Show as s join s.users as u join s.episodes as e join e.torrentIds as tid " +
...
public class SubtitlesImpl extends BaseEntity implements Subtitles {
