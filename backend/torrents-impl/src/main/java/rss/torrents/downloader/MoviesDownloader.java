...
public abstract class MoviesDownloader extends BaseDownloader<MovieRequest, Movie> {
	...
	private Movie getMovieHelper(String imdbId) {
		Movie movie = movieService.findByImdbUrl(imdbId);
		if (movie == null) {
			// if there is no IMDB ID in the original request (meaning it came from the movies job)
			...
			movie = movieService.factory().createMovie();
			movie.setName(imdbParseResult.getName());
			movie.setImdbUrl(imdbId);
			movie.setYear(imdbParseResult.getYear());
			movie.setReleaseDate(imdbParseResult.getReleaseDate());
	...
	private Set<Movie> skipCachedMovies(Set<MovieRequest> requests) {
		...
		for (Torrent torrent : torrents) {
			Movie movie = movieService.find(torrent);
