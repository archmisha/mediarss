...
//	public static final Pattern ENTRY_CONTENT_PATTERN = Pattern.compile("<dl><dt><a href=\"/(\\w+)\">(.*?)</a> &#187; (.*?)</dt>.*?<span class=\"s\">(.*?)</span>.*?<span class=\"u\">(.*?)</span>.*?</dl>");
	...
	private static final String[] KEYWORDS_TO_SKIP = new String[]{"cam", "hdcam", "ts", "dvdscr", "webrip", "hdrip", "brrip", "web dl"};
	...
	@Override
	public Collection<TorrentzResult> downloadMoviesByAge(int days) {
		return downloadByUrl(TorrentzParserImpl.TORRENTZ_LATEST_MOVIES_URL + days + "d");
	}
	...
	protected Set<TorrentzResult> parse(String page) {
		...
		while (matcher.find()) {
			...
			if (!skip) {
				String tmpName = TorrentUtils.normalize(name);
