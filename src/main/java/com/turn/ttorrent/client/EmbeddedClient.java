 ...
 * @author mpetazzoni
 */
public class EmbeddedClient extends Observable implements Runnable,
	 ...
	 */
	public EmbeddedClient(InetAddress address, SharedTorrent torrent) throws IOException {
	...
	public void run() {
		...
		while (!this.stop) {
			...
			try {
				Thread.sleep(UNCHOKING_FREQUENCY*1000);
			} catch (InterruptedException ie) {
	...
	private synchronized void unchokePeers(boolean optimistic) {
		...
		// not connected to.
		TreeSet<SharingPeer> bound = new TreeSet<>(
		...
		int downloaders = 0;
		Set<SharingPeer> choked = new HashSet<>();
	...
	private static class ClientShutdown extends TimerTask {
		...
		ClientShutdown(EmbeddedClient client, Timer timer) {
			this.client = client;
			this.timer = timer;
		}
